...
function authenticateToken(req, res, next) {
  ...
  console.log(`[AUTH] ✓ Authenticated as ${decoded.username} (${decoded.role})`);
  next();
}
...
function authorizeRole(allowedRoles) {
  return (req, res, next) => {
    ...
    console.log(`[AUTH] ✓ Authorization check passed for ${req.user.username}`);
    next();
  };
}
...
function optionalAuth(req, res, next) {
  ...
  if (token) {
    ...
    if (decoded) {
      req.user = decoded;
      req.token = token;
      console.log(`[AUTH] ✓ Optional auth: Authenticated as ${decoded.username}`);
    }
  }
