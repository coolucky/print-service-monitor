...
class PrinterService {
  ...
  loadStatusLogs() {
    try {
      if (fsSync.existsSync(PRINTER_LOGS_FILE)) {
        ...
        console.log(`[PrinterService] Loaded ${logs.length} status logs from file`);
        return logs;
      }
    } catch (error) {
  ...
  saveStatusLogs() {
    try {
      fsSync.writeFileSync(PRINTER_LOGS_FILE, JSON.stringify(this.statusLogs, null, 2), 'utf8');
    } catch (error) {
      console.error('[PrinterService] Error saving status logs:', error.message);
    }
  }
  ...
  checkAndLogStatusChanges(results) {
    ...
    for (const printer of results) {
      ...
      if (wasOnline === undefined) {
        // First time seeing this printer - just record initial state
  ...
  getDefaultStatus(ip, name) {
    return {
      ...
      status: 'Unknown',
      tonerLevels: {
        'Black': Math.floor(Math.random() * 50) + 30 // Random value between 30-80%
      },
      paperStatus: 'Unknown',
      online: false,
  ...
  async getPrintersStatus(printers, papercutSettings = {}) {
    ...
    const statusPromises = printers.map(async (printer) => {
      ...
      console.log(`Ping result for ${name} (${ip}): ${isAlive ? 'online' : 'offline'}`);

      // For online printers, try to fetch toner levels via SNMP
      let tonerLevels = null;
      if (isAlive) {
        try {
          tonerLevels = await snmpService.getTonerLevels(ip, 'public');
          if (tonerLevels && Object.keys(tonerLevels).length > 0) {
            console.log(`SNMP toner data for ${name} (${ip}):`, tonerLevels);
          } else {
            tonerLevels = null;
            console.log(`No SNMP toner data for ${name} (${ip}), will use config fallback`);
          }
        } catch (snmpError) {
          console.warn(`SNMP toner fetch failed for ${name} (${ip}):`, snmpError.message);
          tonerLevels = null;
        }
      }
