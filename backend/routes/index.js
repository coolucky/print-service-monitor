...
function loadPrinterConfigs() {
  try {
    if (fs.existsSync(PRINTERS_FILE)) {
      ...
      // 使用默认配置
      printerConfigs = [
        { name: 'Printer_A', ip: '192.168.1.10' },
        { name: 'Printer_B', ip: '192.168.1.11' },
        { name: 'Printer_C', ip: '192.168.1.12' },
        { name: 'Printer_D', ip: '192.168.1.13' }
      ];
...
router.get('/status/printers', async (req, res) => {
  try {
    console.log('Fetching all printer statuses (public)');
