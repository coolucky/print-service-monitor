    ...
    res.apiSuccess(
      {
        ...
        refreshToken // 可选：也可以生成新的refresh token
      },
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    const forgotPasswordFrom = settings.forgotPasswordEmail || 'noreply@example.com';
    const smtpConfig = {
 ...
 * 测试忘记密码邮件发送（管理员）
 */
router.post('/test-forgot-password-email', authenticateToken, async (req, res) => {
  try {
    if (req.user.role !== 'Administrator') {
 ...
 * 请求体: { email, password }
 */
router.post('/send-password-email', authenticateToken, async (req, res) => {
  try {
    if (req.user.role !== 'Administrator') {
