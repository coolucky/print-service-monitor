...
router.get('/all-printer-status', async (req, res) => {
  try {
    ...
    // 这里需要从上层传入 systemSettings
    const statuses = await printerService.getPrintersStatus(printerConfigs, {});
    
    // Process statuses through alert service
    try {
      alertService.processStatusUpdate(statuses);
    } catch (alertErr) {
      console.warn('[Alert] Error processing status update:', alertErr.message);
    }
